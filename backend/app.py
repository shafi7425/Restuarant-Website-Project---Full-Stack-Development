...
# Allow your frontend origin for all /api routes
CORS(app, supports_credentials=True, resources={
    r"/api/*": {"origins": "https://restuarant-website-project-full-sta-two.vercel.app"}
})
...
def register():
    ...
    if len(username) < 3 or not username.replace(" ", "").isalpha():
        return jsonify({"error": "Username must contain only letters and spaces"}), 400
    ...
    mongo.db.users.insert_one(user_obj)
...
def get_fdishes():
    ...
    for d in dishes:
        ...
        if img_url.startswith("http://") or img_url.startswith("https://"):
            ...
            d["img"] = request.host_url.rstrip("/") + img_url
